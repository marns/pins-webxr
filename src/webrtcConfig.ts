...
export function getPeerOptions(): any {
  ...
  const portRaw = import.meta?.env?.VITE_PEER_PORT as string | undefined;
  const pathEnv = import.meta?.env?.VITE_PEER_PATH as string | undefined;
  const secureEnv = import.meta?.env?.VITE_PEER_SECURE as string | undefined;
  const noIceEnv = import.meta?.env?.VITE_PEER_NO_ICE as string | undefined;
...
export function getOfferEndpoint(): string {
  const full = import.meta?.env?.VITE_OFFER_URL as string | undefined;
...
export function getIceServers(): RTCIceServer[] {
  const json = import.meta?.env?.VITE_ICE_SERVERS as string | undefined;
  if (json) {
    try {
